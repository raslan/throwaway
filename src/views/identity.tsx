...
import { displayedIdentityKeys, fill } from 'src/lib/utils';
...
const ActionButton = ({
  ...
  return (
    <button
      className={`flex items-center duration-300 hover:scale-105 px-2 py-2 rounded-lg w-full my-1 ${className}`}
      onClick={onClick}
    >
      <Icon />
...
const Identity = () => {
  ...
  return (
    <div className='flex w-full h-full'>
      ...
      <div className='w-2/3 max-h-[85%] p-6 gap-3 grid grid-cols-2 overflow-y-scroll'>
        ...
        {Object.entries(identity).map(([key, value], index) => (
          <Fragment key={index}>
            {displayedIdentityKeys?.includes(key) ? (
